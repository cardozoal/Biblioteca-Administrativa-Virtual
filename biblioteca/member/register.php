		...
		if(isset($_POST['m_register']))
		{
			...
			else
			{
				...
				if(mysqli_num_rows($query->get_result()) != 0)
					echo error_with_field("El nombre de usuario que ingresó ya está tomado", "m_user");
				else
				{
					...
					$query->execute();
					if(mysqli_num_rows($query->get_result()) != 0)
						echo error_with_field("Una cuenta ya está registrada con ese correo electrónico", "m_email");
					else
					{
						...
						if($query->execute())
							echo success("Detalles registrados. Se le notificará en el ID de correo electrónico proporcionado cuando sus datos hayan sido verificados");
						else
